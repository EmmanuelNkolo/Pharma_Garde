...
# 1. Update Zoom control
content = content.replace("position: 'bottomright'", "position: 'topright'")

# 2. Add 'Y aller' button to pharmacy cards
repl_str = """          <div class="pharmacy-card-actions" style="display: flex; gap: 8px; flex-wrap: wrap;">
...
content = re.sub(
    r'<div class="pharmacy-card-actions".*?</div>\s*</div>',
    repl_str + '\n        </div>',
    content,
    flags=re.DOTALL
)
